...
async def test_reset_state(dut: SimHandleBase) -> None:
    ...
    await Timer(2, unit=constants.NANOSECONDS)
    assert dut.data_out.value == constants.LOW, (
        f"Reset failed: EXPECTED {constants.LOW}, GOT {dut.data_out.value}"
    )
...
async def test_write_high(dut: SimHandleBase) -> None:
    ...
    await generate_clock_cycle(dut)
    assert dut.data_out.value == constants.HIGH, (
        f"Write High failed: EXPECTED {constants.HIGH}, GOT {dut.data_out.value}"
    )
...
async def test_write_low(dut: SimHandleBase) -> None:
    ...
    await generate_clock_cycle(dut)
    assert dut.data_out.value == constants.LOW, (
        f"Write Low failed: EXPECTED {constants.LOW}, GOT {dut.data_out.value}"
    )
...
async def test_async_rst_during_clk_high(dut: SimHandleBase) -> None:
    ...
    await Timer(1, unit=constants.NANOSECONDS)
    assert dut.data_out.value == constants.LOW, (
        f"Async Reset during CLK High failed: EXPECTED {constants.LOW}, GOT {dut.data_out.value}"
    )
...
async def test_async_rst_during_clk_low(dut: SimHandleBase) -> None:
    ...
    await Timer(1, unit=constants.NANOSECONDS)
    assert dut.data_out.value == constants.LOW, (
        f"Async Reset during clk Low failed: EXPECTED {constants.LOW}, GOT {dut.data_out.value}"
    )
