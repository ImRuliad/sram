...
ifneq ($(shell $(PYTHON_BIN) -c "import cocotb_tools" 2>/dev/null; echo $$?),0)
    # If cocotb_tools is not found in the current python, try to find where it might be
    # or just use the current one if we are in a venv.
    # The error message indicates it's looking for cocotb_tools.config
endif
